...
class Game:
    ...
    def draw(self):
        ...
        postion_text = newfont.render(
            f'x: {self.player.x}, y: {self.player.y}', True, WHITE)
        lvl_text = newfont.render(f'level: {self.player.level}', True, WHITE)
        Hp_text = newfont.render(
            f'hp: {self.player.hp}|{self.player.maxHp}', True, WHITE)
        Atk_text = newfont.render(f'atk: {self.player.atk}', True, WHITE)
        exp_text = newfont.render(
            f'exp: {self.player.curentExp}|{self.player.nextExp}', True, WHITE)
        pot_text = pygame.font.Font('arial.ttf', 12).render(
            f'{self.player.potion}', True, WHITE)
        atk_spd_text = newfont.render(
            f'atk spd: { round(FPS / self.player.magicReduce, 2)}', True, WHITE)
        atk_range_text = newfont.render(
            f'atk range: { math.floor(self.player.magicRange ) * BULLET_SPD}', True, WHITE)
        collide_text = newfont.render(
            f'colliide: {self.colliding}', True, WHITE)
        # Top Left content
        ...
        # Center content
        self.screen.blit(collide_text, (WIN_WIDTH/2 - 100,
                         WIN_HEIGHT - (fontsize + 5)*4))
        self.screen.blit(Hp_text, (WIN_WIDTH/2 - 100,
                         WIN_HEIGHT - (fontsize + 5)*3))
        self.screen.blit(exp_text, (WIN_WIDTH/2 - 100,
                         WIN_HEIGHT - (fontsize + 5)))
    ...
    def gameOver(self):
        ...
        restart_button = Button(
            WIN_WIDTH/2-100, 250, pygame.image.load('./img/resumeBtn - Copy.png'))
    ...
    def pause(self):
        pause = True
        resume_button = Button(WIN_WIDTH/2 - 100, 80,
                               pygame.image.load('./img/resumeBtn.png'))
        back_button = Button(WIN_WIDTH/2 - 100, 200,
                             pygame.image.load('./img/backBtn.png'))
        while pause:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    pause = False
    ...
    def optionScreen(self):
        self.option = True
        soundOn_button = Button(WIN_WIDTH/2-100, 100,
                                pygame.image.load('./img/soundOn.png'))
        soundOff_button = Button(
            WIN_WIDTH / 2 + 20, 100, pygame.image.load('./img/soundOff.png'))
        back_button = Button(WIN_WIDTH/2 - 80, 200,
                             pygame.image.load('./img/backBtn.png'))
        while self.option:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
    ...
    def introScreen(self):
        self.intro = True
        play_button = Button(WIN_WIDTH/2-100, 100,
                             pygame.image.load('./img/playBtn.png'))
        option_button = Button(WIN_WIDTH / 2 - 100, 200,
                               pygame.image.load('./img/optionBtn.png'))
        exit_button = Button(WIN_WIDTH / 2 - 100, 300,
                             pygame.image.load('./img/exitBtn.png'))
        while self.intro:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.intro = False
